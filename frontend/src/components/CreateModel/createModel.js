...
const CreateModelCard = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    use_cases : [],
    code_snippet:'..',
    imageUrl:"https://img.freepik.com/free-photo/ai-technology-microchip-background-digital-transformation-concept_53876-124669.jpg",
  ...
  return (
    <div className="outer-card-create">
      ...
      <h1 style={{marginBottom:"30px", color:"white"}}>Create your own AI model</h1>
      <div className="card-container-create">
        <div className="card-create">
          <h2 className="card-heading-create">Add Details</h2>
          <input type="text" placeholder="Model Name" name='title' onChange={handleChange} required/>
          <input type="text" placeholder="Model Category" name='category' onChange={handleChange} required/>
          <input type="text" placeholder="Provider"   name='provider' onChange={handleChange} required/>
          <input type="text" placeholder="Model-Input-Type"   name='input_type' onChange={handleChange} required/>
          <textarea rows={2} placeholder="Description" name='description' onChange={handleChange} required></textarea>
          <textarea rows={2} placeholder="Use Cases" name='use_cases' onChange={handleChange} required value={formData.use_cases.join(' , ')} readOnly></textarea>
          ...
          />
          <button className="add-usecase-btn" onClick={addUseCase}>Add Use Case</button>
          </div>
        </div>
        <div className="card-create">
      ...
      <button className="button-create" onClick={handleSubmit}>Create Model</button>
      </div>
    </div>
  );
};
